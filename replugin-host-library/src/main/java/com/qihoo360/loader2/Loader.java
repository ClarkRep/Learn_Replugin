...
class Loader {
    ...
    final boolean loadDex(ClassLoader parent, int load) {
        Log.d(RePlugin.MY_TAG, "Loader.loadDex()");
        try {
...
//                }
            ...
            if (mComponents == null) {
                // ComponentList
                Log.d(RePlugin.MY_TAG, "Loader.loadDex() -> 将插件所有的Activity信息存储到：Loader.mComponents.mActivities");
...
//            }
    ...
    private void adjustPluginProcess(ApplicationInfo appInfo) {
        Log.d(RePlugin.MY_TAG, "Loader.adjustPluginProcess() -> 调整插件组件的进程名称，appInfo = " + appInfo);
