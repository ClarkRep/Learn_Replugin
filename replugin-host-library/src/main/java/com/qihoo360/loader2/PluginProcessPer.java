...
class PluginProcessPer extends IPluginClient.Stub {
    ...
    final Class<?> resolveActivityClass(String container) {
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.resolveActivityClass() -> className = " + container);
        ...
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.resolveActivityClass() -> 找到坑位" + container + "映射到的插件Activity：" + activity);
        ...
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.resolveActivityClass() -> 加载插件信息：plugin = " + plugin);
        if (p == null) {
        ...
        try {
            ...
            Log.d(RePlugin.MY_TAG, "PluginProcessPer.resolveActivityClass() -> 使用插件 " + plugin + " 的ClassLoader加载插件中Activity的Class：" + c.getName());
        } catch (Throwable e) {
            if (LOGR) {
    ...
    public String allocActivityContainer(String plugin, int process, String target, Intent intent) throws RemoteException {
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.allocActivityContainer() -> target = " + target);
    ...
    final String bindActivity(String plugin, int process, String activity, Intent intent) {
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.bindActivity() -> activity = " + activity);
        ...
        try {
            ...
            Log.d(RePlugin.MY_TAG, "PluginProcessPer.bindActivity() -> 用插件的ClassLoader预加载真正要跳转的Activity：" + activity);
        } catch (Throwable e) {
            if (LOGR) {
                LogRelease.e(PLUGIN_TAG, e.getMessage(), e);
            }
        ...
        Log.d(RePlugin.MY_TAG, "PluginProcessPer.bindActivity() -> 返回插件Activity " + activity + " 对应的宿主坑位Activity：" + container);
        return container;
    }
