...
//        }
    ...
    final Class<?> loadClass(String className, boolean resolve) {
        Log.d(RePlugin.MY_TAG, "PmBase.loadClass() -> className = " + className);
        // 加载Service中介坑位
        ...
        if (mContainerActivities.contains(className)) {
            Log.d(RePlugin.MY_TAG, "PmBase.loadClass() -> mContainerActivities.contains()");
    ...
    final Plugin loadPlugin(Plugin p, int loadType, boolean useCache) {
        Log.d(RePlugin.MY_TAG, "PmBase.loadPlugin(Plugin p, int loadType, boolean useCache)");
        if (p == null) {
            return null;
        }
